...
| <a name="requirement_ibm"></a> [ibm](#requirement\_ibm) | >= 1.63.0, < 3.0.0 |
...
| <a name="input_skip_iam_authorization_policy"></a> [skip\_iam\_authorization\_policy](#input\_skip\_iam\_authorization\_policy) | Set to true to skip the creation of IAM authorization policies. If set to false, pass in a value for the KMS instance in the existing\_kms\_instance\_guid variable if KMS encryption is enabled. In addition, no KMS policy is created if var.kms\_encryption\_enabled is set to false, and no trusted profile policy is created if var.trusted\_profile\_id is not set. | `bool` | `false` | no |
